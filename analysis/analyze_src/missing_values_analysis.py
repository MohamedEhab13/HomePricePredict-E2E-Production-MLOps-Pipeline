...
# Define abstract class for missing values identification and visualization 
class MissingValuesAnalizerTemplate(ABC):
    def analyze(self, df:pd.date_range):
        """Perform missing values analysis including Identification and Visualization"""
        self.IdentifyMissingValues(df)
        self.VisualizeMissingValues(df)

    @abstractmethod
    def IdentifyMissingValues(self, df: pd.DataFrame):
        """Identify missing values in columns for given DataFrame"""
        pass 

    @abstractmethod
    def VisualizeMissingValues(self, df: pd.DataFrame):
        """Visualize missing values in columns for given DataFrame"""
        pass 


# Implementation of missing value analysis class 
class MissingValuesAnalizer(MissingValuesAnalizerTemplate):
    def IdentifyMissingValues(self, df):
        """Identify missing values in columns for given DataFrame"""
        print("/Missing Values count for each column")
        missing_count = df.isnull().sum()
        print(f"{missing_count[missing_count > 0]}")
