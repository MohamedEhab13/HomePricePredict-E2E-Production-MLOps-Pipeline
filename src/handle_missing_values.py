...
# Define base class for missing value handling strategy 
class MissingValuesHandlerBase(ABC):
    def handle(self, df: pd.DataFrame) -> pd.DataFrame:
        """Handle missing values in given DataFrame based on specific strategy"""
        pass 


# Concrete class for droping missing valuse
class DropMissingValues(MissingValuesHandlerBase):
    def __init__(self, axis=0, thresh=None):
        """
        Get the axis(row/coumns) to be droped if 
        (na) existed in number greater than thresh
        """
        self.axis = axis 
        self.thresh = thresh 
    ...
    def handle(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Drop (na) values in roms/columns based on
        the given axis and thresh 
        """
...
# Concrete class to replace missing values with mean, mode, median or given value
class ReplaceMissingValues(MissingValuesHandlerBase):
    def __init__(self, method="mean", fill_value=None):
        """
        Get method used to replace (na) which could
        be mean, mode, median or constant(fill_value)
        """
        self.method = method 
    ...
    def handle(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Replace (na) in columns with mean or mode of this 
        coulmn or with constant(fill_value)
        """
...
# Concrete class to deal with missing values with different strategies 
class MissingValuesHandler:
    def __init__(self, strategy: MissingValuesHandlerBase):
        """Initalize the strategy for dealing with missing values"""
        self._strategy = strategy

    def set_strategy(self, strategy: MissingValuesHandlerBase):
        """Get new strategy for dealing with missing values"""
